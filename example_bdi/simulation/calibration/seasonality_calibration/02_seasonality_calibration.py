...
burnin_ids = {
    'Gitega': 'e673db63-2ef0-ed11-aa06-b88303911bc1'
}
# set whether this is the first or second set of calibrations for this archetype
round_number = 9  # <-- user needs to specify which round this is to pick up from prior round
if round_number > 1:
...
expname = '%s_%s_BDI_v1_round%i_maxInc%i' % (scen_df.at[scen_index, 'ScenarioName'], rep_admin, round_number,max_incidence)
...
samples_per_iteration = 20  #!!!40  # must be at least 2
...
if later_round:
    round1_fname = os.path.join('%s_%s_BDI_v1_round%i_maxInc%i' % (scen_df.at[scen_index, 'ScenarioName'], rep_admin, (round_number-1), max_incidence), '_plots', 'LL_all.csv')
