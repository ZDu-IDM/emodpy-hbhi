...
extracted_data_dir_country = "C:/Users/moniqueam/Dropbox (IDM)/Malaria Team Folder/data/Burundi/PMI_MOP_caseSeasonality/all_months"
...
base_filepath_raster =  paste0(hbhi_dir, '/burundi_hbhi/SpatialClustering')
sim_output_dir = paste0(hbhi_dir, '/simulation_output/simulations_to_present/BDI_toPresent_allInter_v6')
pop_filepath = paste0(hbhi_dir, '/admin_pop_archetype.csv')


# additional surveillance dataset (downloaded from shared WHO site in 2023)
data <- read_dta('C:/Users/moniqueam/Downloads/incid_month.dta')
# colnames(data)
...
# add additional surveillance dataset
df = data %>% group_by(date) %>%
  summarise(conf_sum = sum(conf),
            maltreat_sum = sum(maltreat))

png(filename=paste0(sim_output_dir,'/_plots/validate_numCases_timeseries_3.png'), width=6, height=4.5, units='in', res=900)
plot(monthly_averages$date, monthly_averages$mean_weekly_cases*52/12, xlim=c(as.Date('2013-06-01'), as.Date('2021-12-01')), ylim=c(min_y_val, max_y_val), type='l', ylab='monthly number of reported cases', xlab='date', bty='L', col=pmi_color, lwd=2)
lines(monthly_values_sum$date, monthly_values_sum$RDT_confirmed, type='l', col=who_color, lwd=2)
lines(as.Date(sim_treated_pop_national$date), sim_treated_pop_national$rescaled_total_treated, type='l', col=sim_color, lwd=2)
lines(df$date, df$conf_sum, type='l', col='red', lwd=2)
abline(v=itn_distribution_dates, col=rgb(0.6,0.5,0.5,0.2), lwd=4)
legend('topleft', c('confirmed cases (from PMI report)', 'RDT confirmed cases (from Bea)', 'simulation treated cases'), col=c(pmi_color, who_color, sim_color), lwd=2, bty='n')
dev.off()
