...
to_present_flag = FALSE
LLIN_2year = FALSE

# set name of country to run
country_name = 'Nigeria'
if(country_name =='Burundi'){
  hbhi_dir = paste0(user_path, '/Dropbox (IDM)/Malaria Team Folder/projects/burundi_hbhi')
  if(LLIN_2year){
    sim_output_base_filepath_2010_allInter = paste0(hbhi_dir, '/simulation_output/simulations_to_present/BDI_2010_2020_allInter_2yLLIN')
  } else{
    sim_output_base_filepath_2010_allInter = paste0(hbhi_dir, '/simulation_output/simulations_to_present/BDI_2010_2020_allInter')
  }
  if(to_present_flag){
    first_year = 2010
    last_year = 2020
    # simulation output directory
    sim_output_filepath = paste0(hbhi_dir, '/simulation_output/simulations_to_present/_counterfactuals')
    # experiment information csv (same as intervention input info file)
    intervention_csv_filename = paste0(hbhi_dir, '/simulation_inputs/_intervention_file_references/Interventions_for_2010_2020_counterfactuals.csv')
    simname_stem = 'BDI_2010_2020'  # only used if don't have experiment names in intervention_csv_filename file
  } else{
    first_year = 2021
    last_year = 2030
    # simulation output directory
    ...
    simname_stem = 'BDI_projection'  # only used if don't have experiment names in intervention_csv_filename file
  }

} else if(country_name =='Burkina'){
  # hbhi_dir = paste(user_path, '/Box/hbhi_burkina', sep='')
  # # filepath for simulation output
  # sim_output_filepath = paste(hbhi_dir, '/simulation_output/2010_to_2020/_v16', sep='')
  # sim_output_base_filepath_2010_allInter = paste(hbhi_dir, '/simulation_output/2010_to_2020/_v16/BF 2010_2020 allInterventions', sep='')
  ...
  sim_output_base_filepath_2010_allInter = paste0(hbhi_dir, '/simulation_output/simulations_to_present/NGA_toPresent_allInter_v12')
  ...
  if(to_present_flag){
    ...
    # experiment information csv (same as intervention input info file)
    intervention_csv_filename = paste0(hbhi_dir, '/simulation_inputs/_intervention_file_references/Interventions_to_present.csv')
    simname_stem = ''  # only used if don't have experiment names in intervention_csv_filename file
  } else{
    first_year = 2022
    last_year = 2029
    # simulation output directory
    # sim_output_filepath = paste0(hbhi_dir, '/simulation_output/simulations_future/v3')
    sim_output_filepath = paste0(hbhi_dir, '/simulation_output/simulations_future/v5_GF_PAAR')
    # experiment information csv (same as intervention input info file)
    intervention_csv_filename = paste0(hbhi_dir, '/simulation_inputs/_intervention_file_references/Interventions_for_projections_GF_PAAR.csv')
    # intervention_csv_filename = paste0(hbhi_dir, '/simulation_inputs/_intervention_file_references/Interventions_for_projections.csv')
    simname_stem = ''  # only used if don't have experiment names in intervention_csv_filename file
    
    
    
    # !!! Remove after finishing work-around processing
    # intervention_csv_filename = paste0(hbhi_dir, '/simulation_inputs/_intervention_file_references/Interventions_for_projections_forPostProcessing.csv')
    # sim_output_filepath = paste0(hbhi_dir, '/simulation_output/simulations_future/v3/_separateSeeds')
    
  }
} else if (country_name=='Guinea'){
  # hbhi_dir = paste(user_path, '/Box/hbhi_guinea', sep='')
  # # filepath for simulation output
  # sim_output_filepath = paste(hbhi_dir, '/simulation_output/2012_to_2021_v210326', sep='')
  # sim_output_base_filepath_2010_allInter = paste(hbhi_dir, '/simulation_output/2012_to_2021_v210326/GN 2012-2021', sep='')
} else{
...
script_base_filepath = paste(user_path, '/Documents/malaria-nga-snt22/simulation/IPTp_mortality_postprocessing', sep='')
####################################################################################################################################################
...
iptp_estimates_filename = paste0(hbhi_dir,'/simulation_inputs/IPTp/estimated_past_IPTp_each_DS.csv')
iptp_estimates_ci_l_filename = paste(hbhi_dir,'/simulation_inputs/IPTp/estimated_ci_l_past_IPTp_each_DS.csv', sep='')
iptp_estimates_ci_u_filename = paste(hbhi_dir,'/simulation_inputs/IPTp/estimated_ci_u_past_IPTp_each_DS.csv', sep='')
iptp_project_trajectory_filename = paste(hbhi_dir,'/simulation_inputs/IPTp/projected_future_trajectory_IPTp_each_DS.csv', sep='')
# filename for estimated number of IPTp doses taken by IPTp recipients in each year
iptp_dose_number_filename = paste(hbhi_dir,'/simulation_inputs/IPTp/estimated_past_num_doses.csv', sep='')
...
ipti_adj_U1_filename = paste(sim_output_filepath, '/IPTi_adjustment.U1.csv', sep='')   # ages: U1
ipti_adj_U5_filename = paste(sim_output_filepath, '/IPTi_adjustment.csv', sep='')        # ages: U5
ipti_adj_Uall_filename = paste(sim_output_filepath, '/IPTi_adjustment.Uall.csv', sep='') # all ages
# if those files don't exist, see whether the CI version does. if so, convert to preferred format
ipti_adj_U1_CI_filename = paste(sim_output_filepath, '/IPTi_adjustment_U1_withCI.csv', sep='')
...
if(!file.exists(ipti_adj_U1_filename)){
  if(file.exists(ipti_adj_U1_CI_filename)){
    convert_CI_to_mean_format(filename_CI=ipti_adj_U1_CI_filename, filename_mean=ipti_adj_U1_filename)
  }
}
...
parameter_filepath = paste0(hbhi_dir, '/simulation_inputs/parameters_mortality_MiP_IPTp_IPTi.csv')
...
if(file.exists(intervention_csv_filename)){
...
if(any(scenario_adjustment_info$future_projection_flag %in% c('True','False'))){
  scenario_adjustment_info$future_projection_flag = as.logical(toupper(scenario_adjustment_info$future_projection_flag))
}
if(any(scenario_adjustment_info$PMC_postprocess_flag %in% c('True','False'))){
  scenario_adjustment_info$PMC_postprocess_flag = as.logical(toupper(scenario_adjustment_info$PMC_postprocess_flag))
}
if(any(scenario_adjustment_info$IRS_admins_only %in% c('True','False'))){
  scenario_adjustment_info$IRS_admins_only = as.logical(toupper(scenario_adjustment_info$IRS_admins_only))
}
if(any(scenario_adjustment_info$IPTi_admins_only %in% c('True','False'))){
  scenario_adjustment_info$IPTi_admins_only = as.logical(toupper(scenario_adjustment_info$IPTi_admins_only))
}
if(any(scenario_adjustment_info$only_run_admin_subset %in% c('True','False'))){
  scenario_adjustment_info$only_run_admin_subset = as.logical(toupper(scenario_adjustment_info$only_run_admin_subset))
}
...
# for(i_scen in c(10)){#1, 2,3, 4,5,6,7,8)){
  ### - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ###
  ...
  scenarioName_IPTi = scenarioName_cur  #scenario_adjustment_info$IPTi_scenario_name[i_scen]
  ...
  if('IRS_admins_only' %in% colnames(scenario_adjustment_info)){
    admin_subset_flag1 = scenario_adjustment_info$IRS_admins_only[i_scen]
  } else admin_subset_flag1 = FALSE
  if ('IPTi_admins_only' %in% colnames(scenario_adjustment_info)){
    admin_subset_flag2 = scenario_adjustment_info$IPTi_admins_only[i_scen]
  } else admin_subset_flag2 = FALSE
  if('only_run_admin_subset' %in% colnames(scenario_adjustment_info)){
    admin_subset_flag3 = scenario_adjustment_info$only_run_admin_subset[i_scen]
  } else admin_subset_flag3 = FALSE
  admin_subset_flag = admin_subset_flag1 | admin_subset_flag2 | admin_subset_flag3
  ...
  # filename for simulation output on all U1 cases and PfPR (will be adjusted with IPTi outcomes and used for plotting)
  sim_output_U1PfPR_filename_cur = paste(sim_output_base_filepath, "/U1_PfPR_ClinicalIncidence_severeTreatment.csv", sep='')
  # filename for simulation output on all U5 cases and PfPR (will be adjusted with IPTi outcomes and used for plotting)
  sim_output_U5PfPR_filename_cur = paste(sim_output_base_filepath, "/U5_PfPR_ClinicalIncidence_severeTreatment.csv", sep='')
  # filename for simulation output on all age monthly cases and PfPR (will be adjusted with MiP, IPTp, and IPTi outcomes and used for plotting)
  ...
  if(file.exists(sim_output_filename)){
    ...
    # if simulation only contains a subset of IRS admins, get names of admins
    if (admin_subset_flag){
      admin_subset = unique(sim_output_all_runs$admin_name)
    } else admin_subset = NA

    # replace the column name LGA with admin_name, as is expected later in the script
    if('LGA' %in% colnames(sim_output_all_runs)){
      colnames(sim_output_all_runs)[which(colnames(sim_output_all_runs) == 'LGA')] = 'admin_name'
    }
    if('DS_Name' %in% colnames(sim_output_all_runs)){
      colnames(sim_output_all_runs)[which(colnames(sim_output_all_runs) == 'DS_Name')] = 'admin_name'
    }
    ...
    IPTp_coverages = get_IPTp_coverages(iptp_estimates_filename=iptp_estimates_filename, iptp_dose_number_filename=iptp_dose_number_filename, 
                                        ...
                                        iptp_project_trajectory_filename=iptp_project_trajectory_filename, admin_subset_flag=admin_subset_flag, admin_subset=admin_subset)
    iptp_coverage_df = IPTp_coverages[[1]]
    iptp_dose_number = IPTp_coverages[[2]]
    admin_names = IPTp_coverages[[3]]
    ...
    sim_output_U1PfPR_filename_IPTi = gsub( '.csv', paste('_withIPTi.csv', sep=''), sim_output_U1PfPR_filename_cur)
    ...
    calculate_pregnancy_values_across_runs(sim_output_all_runs=sim_output_all_runs, first_year=first_year, last_year=last_year, admin_names=admin_names,
                                           ...
                                           future_projection_flag=future_projection_flag, admin_subset_flag=admin_subset_flag, admin_subset=admin_subset, overwrite_files_flag=FALSE)
    ...
    if(scenario_adjustment_info$PMC_postprocess_flag[i_scen]){
      print('... adjusting for IPTi')
      adjust_sim_output_for_ITPi(scenarioName_IPTi=scenarioName_IPTi,
                                 ipti_adj_U1_filename=ipti_adj_U1_filename,
                                 ipti_adj_U5_filename=ipti_adj_U5_filename,
                                 ipti_adj_Uall_filename=ipti_adj_Uall_filename,
                                 sim_output_U1PfPR_filename_cur=sim_output_U1PfPR_filename_cur,
                                 ...
                                 sim_output_U1PfPR_filename_IPTi=sim_output_U1PfPR_filename_IPTi,
      ...
      sim_output_U1PfPR_filename_old=sim_output_U1PfPR_filename_IPTi
      ...
      sim_output_U1PfPR_filename_old=sim_output_U1PfPR_filename_cur
    ...
    adjust_sim_output_mortality(cfr_severe_treated=cfr_severe_treated, # case fatality rate for severe, treated cases
                                ...
                                sim_output_U1PfPR_filename_old=sim_output_U1PfPR_filename_old,
                                ...
                                cases_U1_colname=cases_U1_colname,
                                cases_U5_colname=cases_U5_colname,
                                cases_all_colname=cases_all_colname,
                                severe_U1_colname=severe_U1_colname,
                                severe_U5_colname=severe_U5_colname,
                                severe_all_colname=severe_all_colname,  # does not include maternal severe (due to order of rescaling between IPTp and IPTi)
                                severe_treated_U1_colname=severe_treated_U1_colname,
                                severe_treated_U5_colname=severe_treated_U5_colname,
                                severe_treated_all_colname=severe_treated_all_colname,  # includes treatment from non-maternal and maternal severe malaria
