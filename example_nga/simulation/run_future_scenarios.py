...
from simulation.helpers_sim_setup import update_basic_params, load_master_csv, habitat_scales, set_up_hfca, ReportSimulationStats
...
num_burnin_seeds = 5  # number of seeds run during "to-present" simulations
ser_date = 12*365
population_size = 6000  # needs to match burnin simulation population size
itn_anc_adult_birthday_years = [20, 22, 24, 26, 28]
sp_resist_day1_multiply = 0.82

if population_size == 6000:  # generated from 2010-2021 run
    burnin_id = ''  # _v13
else:
    warnings.warn('no experiment on file for this population size. attempting with the experiment for pop=6000')
    burnin_id = ''


scenario_fname = os.path.join(project_path, 'simulation_inputs', '_intervention_file_references', 'Interventions_for_projections_GF_PAAR.csv')
...
seed = int(round(scen_df.at[scen_index, 'seed']))
expname = '%s_v6_seed%i' % (expname, seed)
...
if run_with_debug:
    stats_report = ReportSimulationStats()
    # add report to look at persistent interventions per person
    cb.add_reports(stats_report)
...
hab_scale_factor_fname = os.path.join(project_path, 'simulation_inputs', 'larval_habitats', 'larval_habitat_multipliers_v2.csv')
...
builder = ModBuilder.from_list([[ModFn(set_up_hfca, hfca=my_admin,
                                ...
                                # for my_admin in ['Gombe']
                                # for my_admin in ["Zaki", "Brass"]
                                # for my_admin in df.index[range(3)]
                                for my_admin in df.index
                                # for x in range(num_seeds)
                                for x in range(seed, (seed+num_seeds))
                                # for x in range(1)
                                ])
