...
def load_ref_data():
    dhs_pfpr_fname = os.path.join(project_path, 'estimates_from_DHS', 'DHS_admin_monthly_microscopy.csv')
...
def plot_graphs(mult_param, score_df, df, dhs_df, fig1, axes1, fig2, axes2, row, col, hfca, run_number):
    ...
    if len(axes1.shape)>1:
        ...
        # TODO: might want to add error bars with .errorbar()... could consider normal approximation interval https://en.wikipedia.org/wiki/Binomial_proportion_confidence_interval
...
if __name__ == "__main__":
    ...
    working_dir = os.path.join(project_path, 'simulation_output', 'transmission_intensity_calibration', expt_name)
    ...
    habitats_fname = os.path.join(project_path, 'simulation_inputs', 'larval_habitats',
                                  'larval_habitat_multipliers_v2.csv')
