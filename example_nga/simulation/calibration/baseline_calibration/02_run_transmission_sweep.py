...
from simulation.helpers_sim_setup import update_basic_params, set_up_hfca,  load_master_csv, habitat_scales
...
expname = 'PfPR_calibration_main_sweep_NGA_v6_part2'
burnin_id = '121c897e-9ebe-ed11-aa03-b88303911bc1'  # generated from serialize_transmission_sweep
...
ser_date = 50*365
years = 12  # beginning of 2010 - end of 2021
start_year = 2010
...
sweepspace = {
    ...
    # maxInc50: rep_admin: [np.logspace(-1.5, 2, 25)[ii] for ii in [0, 2, 4, 6, 8, 10, 12]] for rep_admin in lhdf['archetype'].values  # make sure this matches the values from serialize_transmission_sweep
    # maxInc50: rep_admin: [np.logspace(-1.5, 2, 25)[ii] for ii in [14, 16, 18, 20, 22, 24]] for rep_admin in lhdf['archetype'].values  # make sure this matches the values from serialize_transmission_sweep
    # rep_admin: [np.logspace(-1.6, 1.7, 25)[ii] for ii in [0, 2, 4, 6, 8, 10, 12]] for rep_admin in lhdf['archetype'].values  # make sure this matches the values from serialize_transmission_sweep
    rep_admin: [np.logspace(-1.6, 1.7, 25)[ii] for ii in [14, 16, 18, 20, 22, 24]] for rep_admin in lhdf['archetype'].values  # make sure this matches the values from serialize_transmission_sweep
}
...
builder = ModBuilder.from_list([[ModFn(set_up_hfca, hfca=my_admin, archetype_hfca=df.at[my_admin, 'seasonality_archetype'],
                                ...
                                # for my_admin in ['Zurmi']
                                # for my_admin in ['Burutu']
