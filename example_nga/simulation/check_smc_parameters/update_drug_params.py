...
def update_drugs_from_list(drug_nested_param_value_list, malaria_drug_params=Malaria_Drug_Params):
    # Updates >=1 drug parameters based on the values in drug_nested_param_value_list.
    # drug_nested_param_value_list is a list, where each element specifies a parameter to change and its target value.
    # Each element of drug_nested_param_value_list needs to be a list where the first element is a list giving the nested set of parameter names and the second element is the target value.

    # iterate through items in the drug_nested_param_value_list
    for ii in range(len(drug_nested_param_value_list)):
        list_drug_param = drug_nested_param_value_list[ii][0]
        value = drug_nested_param_value_list[ii][1]
        # list_drug_param takes arguments as a list of parameter fields and the values
        if len(list_drug_param) == 1:
            malaria_drug_params[list_drug_param[0]] = value
        elif len(list_drug_param) == 2:
...
def update_drug_config_from_list(cb, drug_nested_param_value_list):
    cb.update_params({
        "Malaria_Drug_Params": update_drugs_from_list(drug_nested_param_value_list=drug_nested_param_value_list)
    })
    return({'new drug value': drug_nested_param_value_list[0][1]})